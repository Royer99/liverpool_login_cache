...
`_id` point read must stay EXPRESS/IDHACK. The `email_1` query index
(`ensure_indexes`, added 2026-08-05 at customer request, partial by
necessity — zset arrays make plain `value.*` indexes multikey) serves
`find_by_email` only and must never appear in `get_session`'s plan. Do NOT
index `value.accessToken`: ~1 KB JWT keys make the build/index prohibitively
expensive (tried and dropped 2026-08-05). A device+recency compound was
also tried and cut 2026-08-05: multikey broke its sort (in-memory sort over
~3.4M fetched docs); the `sessions_by_device` op was removed with it.
